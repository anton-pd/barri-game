...
Головна інструкція для Куратора справи (LLM). Обов'язково містить:

1. **Хто ти** — Куратор справи для детективного горору, назва сценарію, місце і час
...
```
Ти — Куратор справи для настільної RPG-розслідування. Ведеш сценарій «[Назва]». [Місто, рік].
...
```json
{
  ...
  "response": "Куратор справи робить [конкретний наслідок або перешкода, яка зупиняє але не ламає гру]"
}
```
...
Мінімум 4-5 сюжетних вузлів які МАЮТЬ відбутися незалежно від дій гравців. Куратор справи знаходить можливість.
...
- `introHint` — одне речення для Куратора справи; передається в перший запит і очищається після інтро
...
```json
{
  ...
  "systemPrompt": "Ти — Куратор справи для настільної RPG-розслідування. Ведеш сценарій «Туман Інсмута». Інсмут, 1922.\n\nПРИРОДА ЖАХУ: ...\n\nГОЛОВНИЙ ПРИНЦИП: Реагуй на дії — не веди...",
  "railguards": [
    {
