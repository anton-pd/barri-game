...
## 2026-04-22 — Keeper logic + prompt review (staging)
**Scope:** Pulled latest `origin/staging` (`Already up to date`) and reviewed keeper runtime/prompt stack in `src/app/api/ai/route.ts`, `src/lib/prompts.ts`, `src/lib/rulesets.ts`, plus scenario `systemPrompt` blocks.

**Main findings:**
- `isExplicitImageRequest()` is too broad: bare nouns like `map`, `letter`, `photo`, `image` trigger mandatory `[IMAGE:...]` injection even when the player is just interacting with an object, not asking to see it.
- `isPassiveMessage()` is effectively Ukrainian-only and also marks many short English action messages as passive via `message.length < 20`, which can incorrectly push the keeper into proactive hinting.
- Prompt stack has real duplication and one conflict: scenario `systemPrompt` + localized response-style blocks repeat the same anti-leading rules, while `rulesets.ts` still tells the model to "offer" a substitute skill when the original one is missing.
- CoC roll wording is slightly misaligned with the visible sheet: prompt examples use `Стійкість` / `Stability`, while runtime stat lines expose `SAN`, and Luck naming is also mixed.

**Overall:** architecture is strong (ruleset/static/dynamic split, server-owned tags/state, met-only NPC filtering), but prompt consistency is now the main maintainability risk.
- Prompt audit follow-up tasks created in Linear (AI Improvements, assigned to Codex): `ANT-84` explicit image-request detection, `ANT-85` passive-turn heuristic localization, `ANT-86` CoC fallback guidance vs reactive keeper policy, `ANT-87` SAN/Luck terminology normalization, `ANT-88` canonical prompt ownership / deduplication.

## 2026-04-22 — GitHub secret / abuse surface audit
**Scope:** Audited local clone + fetched Git history for `origin/main` and `origin/staging` (the only remote heads on GitHub) for committed secrets, tracked env/key files, and routes that can proxy paid APIs.

**Findings:**
- No literal API keys, private keys, or tracked `.env*` files found in current tree or fetched GitHub branch history. One deleted `DEPLOY.md` match was a placeholder (`PASSWORD` / `${POSTGRES_PASSWORD}`), not a real secret.
- High-risk abuse surface exists in unauthenticated paid API proxy routes: `src/app/api/tts/route.ts`, `src/app/api/stt/route.ts`, `src/app/api/image/route.ts`, `src/app/api/scenarios/[id]/ambient/route.ts`, `src/app/api/scenarios/[id]/images/route.ts`.
- Admin routes `src/app/api/admin/settings/route.ts` and `src/app/api/admin/pricing/route.ts` currently allow reads/writes without auth checks.
- No general request rate limiting found around the paid-generation endpoints, so scripted abuse would be easy if these routes are publicly reachable.

**Recommended next steps:** require auth + ownership/admin checks on all generation/admin routes, add rate limiting, and review provider usage logs for unexpected spend before deciding whether key rotation is needed.

## 2026-04-23 — Noir chat design-system rollout plan
**Scope:** Audited current landing noir system, session chat implementation, and competitor positioning for AI/VTT RPG products.

**Findings:**
- Landing already defines the canonical noir-dossier system in `src/app/landing.css`: ink/paper/amber/blood tokens, film grain, vignette, scratches, typewriter/serif/oldprint/blackletter font roles, dossier cards, stamps, evidence-red CTA shadows.
- `src/app/session/[id]/layout.tsx` already imports `landing.css` + `chat.css` and wraps sessions in `landing-root`, so chat has access to the design tokens and fonts.
- `src/app/session/[id]/chat.css` already contains a first noir chat stylesheet, but `src/components/GameChat.tsx` still renders most core UI with Tailwind `stone`/`amber` utility classes. The main gap is wiring semantic chat classes into JSX and extending the system to sidebar/controls.
- Competitor scan: AI RPG/VTT products emphasize persistent world memory, real rules/dice, tactical maps/materials, integrated AI GM, voice/ambient immersion, and cross-device play. Barri should differentiate with a focused Call of Cthulhu/noir investigation workspace rather than a generic fantasy dashboard.

**Linear follow-up tasks created in AI Improvements, assigned to Codex:**
- `ANT-89` Noir chat foundation: wire landing design tokens into GameChat.
- `ANT-90` Noir chat case files: redesign sidebar as evidence dossier.
- `ANT-91` Noir chat instruments: restyle stats, dice, voice, and controls.
- `ANT-92` Noir chat atmosphere: add restrained motion and ambient visual states.
- `ANT-93` Noir chat QA: responsive, contrast, and staging verification pass.

## 2026-04-23 — ANT-89 noir chat UX migration implementation
**Scope:** Implemented the approved staging branch UX migration for the main session chat on `feature/ANT-89-noir-chat-ux`.

**Solution:**
- Added session route styling entrypoints: `src/app/session/[id]/layout.tsx` loads the same noir font variables and `landing-root` tokens used by landing/sessions, and imports `chat.css`.
- Reworked `GameChat.tsx` core UI from Tailwind `stone` utilities to semantic `chat-*` / `case-*` classes: shell, header, settings, messages, bubbles, replay/debug, queue, inventory strip, physical dice hint, input, read-only state, modals, and sidebar shell.
- Rebuilt `src/app/session/[id]/chat.css` as a full noir-dossier product stylesheet: ink/paper/amber/blood surfaces, Keeper paper narration, dark investigator replies, NPC evidence bubbles, case-file sidebar, tabbed evidence cards, modals, responsive mobile drawer, low-motion grain, and `prefers-reduced-motion` fallbacks.
- Restyled `VoiceButton` and `DiceRoller` onto the same design system. DiceRoller also drops the old synchronous `setState` inside `useEffect`; parent key remount already resets rolls.

**Verification:**
- `npm run lint` passes with existing warnings only (`<img>` warnings and one unrelated unused `createCampaign` import).
