...
### Fixed
- **English games now use English scenario content throughout (ANT-192).** All nine live cases have native English player-facing descriptions, briefings, role sheets, inventory copy, and location names. Existing English sessions are localized without changing character state or game mechanics, Spanish scenario copy falls back to English instead of Ukrainian, and newly generated cases must include an English localization alongside the canonical Ukrainian scenario.
- **Demo transcript speaker labels follow the selected language (ANT-210).** Player and Case Curator labels no longer fall back to English after the first Ukrainian or Spanish AI turn.
- **AI turns are durable, single-flight, and bounded (ANT-203/204).** Logical retries replay the committed result instead of paying twice, only one provider call may run per session, messages and world state commit atomically, and provider calls are cancelled within a 120-second budget under a 180-second lease. Turn bodies and actions are bounded and access is revalidated after the lease is won.
- **Production dependency security baseline for beta (ANT-208).** Updated Next.js to the current 16.2 security release, refreshed the email SDK, moved the eval-only Anthropic SDK out of the production dependency set, and pinned patched DOMPurify, PostCSS, and Sharp builds. The production dependency audit now reports zero known vulnerabilities.
- **Session failures now show a recoverable state (ANT-211).** Session/catalog outages display an inline retry instead of an empty archive, failed session creation keeps the modal and draft intact, expired authentication still redirects to sign-in, and server-side session DB errors use a safe unavailable screen instead of a false 404.
- **Landing no longer displays unverified testimonials (ANT-213).** Removed unverified player names, quotations, testimonial navigation, and their unused styling; the landing JSON-LD contains no review or rating claims.
- **The demo now helps players begin without starting the clock prematurely (ANT-212).** It offers three localized starter actions plus a free-form alternative only before the first turn, starts the 15-minute limit only on that intentional first turn, and gives email capture a concise privacy notice linking to the existing policy and terms.
- **Deploys now stop on quality or readiness failures (ANT-208).** CI performs a clean install, tests, typecheck, lint, and a production build before it can SSH to the VPS; after deployment it requires a valid `200` readiness response confirming database and scenario-source availability.
- **Session and shared-browser privacy (ANT-202).** Session data, messages, AI turns, and completion actions require the exact current account owner (with current DB admins retaining intentional support access); ownerless legacy rows are denied to normal users. Read-only browser snapshots are account-scoped and cleared on logout or account deletion.
- **Public demo and auth abuse protection (ANT-201).** Demo AI usage now has server-side session/IP quotas. Login, signup, password recovery, verification, and waitlist flows enforce bounded request sizes and return `429` with `Retry-After` when throttled.
- **Scenario catalog no longer leaks Keeper-only material (ANT-200).** Public scenario listings now expose only card/session-creation metadata and explicitly mapped investigator fields; prompts, secrets, clues, event plans, variants, finales, and evidence-generation data remain server-side. Pre-generated scenario evidence is admin-only until an unlock model exists, while evidence revealed during a live investigation remains available.
...
- **Cookiebot no longer risks blocking app hydration (ANT-199).** Removed Cookiebot's auto-blocking mode, which could intercept Next.js scripts and leave the interface non-interactive. The production-only Cookiebot banner remains active, while PostHog still initializes only after explicit analytics/statistics consent.
- **Analytics now redacts URL credentials (ANT-205).** PostHog strips every query string and URL fragment before sending pageviews or standard event properties, so reset links, invitation links, email values, and future sensitive query parameters cannot be recorded while route path analytics remains useful.
- **AI engine tier is now selected only by the server (ANT-185).** `/api/ai` ignores forged client `aiProvider` values and resolves the base/pro tier from the current admin setting. Access-cap and admin ownership checks now use the user's current database role as well.
- **Paid media endpoints now enforce current access, ownership and limits (ANT-198).** TTS, speech recognition and dynamic image generation require an approved current DB user who owns the session, apply the daily cost gate and bounded payloads, and use provider timeouts. Static scenario image materialization is admin-only.
- **Shared scenario source is mutation-locked by default (ANT-207).** Staging can no longer save, generate, or delete the production-shared scenario JSON. Source mutations require an explicit, production-host-scoped maintenance enable; cache asset generation and all scenario reads remain available.
