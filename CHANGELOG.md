...
### Added
- **Staging Environment** — створено ізольоване середовище розробки `staging.barrigame.es` на порту `3001` (папка `/opt/apps/cthulhu`). Дозволяє тестувати зміни без ризику для основного сайту.
- **Ready for Deployment** — новий стан у Linear. AI деплоїть таску на пром лише після того, як вона переведена в цей стан та заассайнена назад на AI.
- **AI Improvements** — автоматичне створення тасків для запропонованих ідей та аудитів у відповідній колонці Linear (одна таска на фічу).
- **Identity segregation** — чіткий поділ тасок між Claude та Codex (кожен бере лише заассайнені на себе завдання).

### Changed
- **Git Branching Strategy** — основною гілкою розробки стала `staging`. Всі фічі розробляються у `feature/ANT-XXX` від `staging`. `main` використовується виключно для стабільного продакшн-релізу.
- **Linear Workflow** — оновлено правила ассайменту: AI ассайнить таску на себе для роботи, перекидає на ревью (Anton) → повертається AI для деплою → переводиться в Done.
